...
function BuyerOrdersContent() {
  ...
  useEffect(() => {
    ...
    if (paymentSuccess) {
      toast({
        ...
        description: "Your payment has been reconciled. Fulfillment is now being tracked via trace_id.",
      });
  ...
  const fetchOrders = async (pageToFetch: number = 0) => {
    ...
    try {
      ...
      log.error("Ledger fetch error:", undefined, error);
    } finally {
      setIsLoading(false);
      loadingSetter(false);
    }
      ...
      <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Review system-verified transactions and monitor fulfillment status.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <>
                ...
                <div className="text-center py-20 border-2 border-dashed rounded-3xl">
                    ...
                    <h3 className="text-xl font-bold">Ledger Empty</h3>
                    <p className="text-muted-foreground mt-2 max-w-xs mx-auto text-sm">No transactions have been recorded for your account. Purchase intent will appear here post-fulfillment.</p>
                    <Button asChild className="mt-6" size="lg">
                        <Link href="/products">Browse Active Catalog</Link>
                    </Button>
                </div>
            )}
          </CardContent>
